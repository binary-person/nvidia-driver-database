...
const LOOKUP_ROW_MANUAL_OVERRIDES = Object.freeze({});
...
function assertBrowserLookupMappings(db) {
  ...
  `).pluck().all().filter((name) => !hasResolvableBrowserProductLookupName(name, productLookupNames)).slice(0, 10);
...
function hasResolvableBrowserProductLookupName(productName, productLookupNames) {
  return productLookupNames.has(productName)
    || productLookupNames.has(getCanonicalBrowserProductName(productName));
}
...
function createRepository(db, databaseFilePath = "", options = {}) {
  let contentChanged = false;
  const lookupRowManualOverrides = options.lookupRowManualOverrides || LOOKUP_ROW_MANUAL_OVERRIDES;
  ...
  const statements = {
    ...
    `),
    updateLookupValueByTypeAndValue: db.prepare(`
      ...
      SET
        ...
        is_select_less = @isSelectLess,
        extra_attrs_json = @extraAttrsJson,
  ...
  function parseStoredLookupExtraAttrs(jsonText) {
  ...
  function normalizeLookupRowForComparison(row) {
    return {
      typeId: Number(row.typeId),
      lookupName: String(row.lookupName),
      value: String(row.value),
      name: String(row.name),
      parentTypeId: row.parentTypeId === null || row.parentTypeId === undefined || row.parentTypeId === ""
        ? null
        : Number(row.parentTypeId),
      parentValue: row.parentValue === null || row.parentValue === undefined ? "" : String(row.parentValue),
      code: row.code === null || row.code === undefined ? "" : String(row.code),
      requiresProduct: row.requiresProduct === null || row.requiresProduct === undefined ? "" : String(row.requiresProduct),
      isSelectLess: row.isSelectLess === null || row.isSelectLess === undefined ? "" : String(row.isSelectLess),
      ordinal: Number(row.ordinal || 0),
      extraAttrs: sortObjectKeys(row.extraAttrs || {}),
    };
  }

  function normalizeStoredLookupRow(row, typeId, lookupName) {
    return normalizeLookupRowForComparison({
      typeId,
      lookupName,
      value: row.value,
      name: row.name,
      ...
      isSelectLess: row.is_select_less,
      ordinal: row.ordinal,
      extraAttrs: parseStoredLookupExtraAttrs(row.extra_attrs_json),
    });
  }

  function getLookupRowOverride(typeId, value) {
    return lookupRowManualOverrides[`${String(typeId)}:${String(value)}`] || null;
  }

  function applyLookupRowOverride(row) {
    const override = getLookupRowOverride(row.typeId, row.value);
    if (!override) {
      return {
        row,
    ...
    return {
      row: normalizeLookupRowForComparison({
        ...row,
        ...override,
        extraAttrs: override.extraAttrs === undefined ? row.extraAttrs : override.extraAttrs,
      }),
      overrideApplied: true,
    };
  }

  function areLookupRowsEquivalent(left, right) {
    return left.lookupName === right.lookupName
      && left.value === right.value
      && left.name === right.name
      && left.parentTypeId === right.parentTypeId
      && left.parentValue === right.parentValue
      && left.code === right.code
      && left.requiresProduct === right.requiresProduct
      && left.isSelectLess === right.isSelectLess
      && JSON.stringify(left.extraAttrs) === JSON.stringify(right.extraAttrs);
  }

  function formatLookupDiffValue(value) {
    return value === "" ? "(empty)" : String(value);
  }

  function formatLookupRowParent(row) {
    return row.parentTypeId !== null && row.parentValue !== ""
      ? `${row.parentTypeId}:${row.parentValue}`
      : "(none)";
  }

  function describeLookupAttributeChanges(previousRow, nextRow) {
    const changes = [];
    if (previousRow.code !== nextRow.code) {
      changes.push(`code ${formatLookupDiffValue(previousRow.code)} -> ${formatLookupDiffValue(nextRow.code)}`);
    }
    if (previousRow.requiresProduct !== nextRow.requiresProduct) {
      changes.push(
        `requiresProduct ${formatLookupDiffValue(previousRow.requiresProduct)} -> ${formatLookupDiffValue(nextRow.requiresProduct)}`
      );
    }
    if (previousRow.isSelectLess !== nextRow.isSelectLess) {
      changes.push(
        `isSelectLess ${formatLookupDiffValue(previousRow.isSelectLess)} -> ${formatLookupDiffValue(nextRow.isSelectLess)}`
      );
    }

    const previousExtraAttrs = JSON.stringify(previousRow.extraAttrs);
    const nextExtraAttrs = JSON.stringify(nextRow.extraAttrs);
    if (previousExtraAttrs !== nextExtraAttrs) {
      changes.push(`extraAttrs ${previousExtraAttrs} -> ${nextExtraAttrs}`);
    }

    return changes;
  }

  function buildLookupStoredContentHash(rows) {
    return hashLookupEntries(rows.map((row) => ({
      code: row.code,
      extraAttrs: row.extraAttrs,
      isSelectLess: row.isSelectLess,
      lookupName: row.lookupName,
      name: row.name,
      ordinal: row.ordinal,
      parentTypeId: row.parentTypeId,
      parentValue: row.parentValue,
      requiresProduct: row.requiresProduct,
      typeId: row.typeId,
      value: row.value,
    })));
  }

  function buildLookupMergeDiagnostics(existingRows, candidateRows, storedRows, insertedRows, overriddenRows, preservedRows) {
    const insertedSample = insertedRows.slice(0, 5).map((row) => `${row.value}=${row.name}`);
    const overriddenSample = overriddenRows.slice(0, 5).map((row) => `${row.value}=${row.after.name}`);
    const omittedExistingSample = [];
    const sameValueNameChangedSample = [];
    const sameValueParentChangedSample = [];
    const sameValueAttributeChangedSample = [];
    let preservedChangedCount = 0;

    preservedRows.forEach(({ existingRow, candidateRow }) => {
      omittedExistingSample.push(`${existingRow.value}=${existingRow.name}`);
    });

    candidateRows.forEach((candidateRow) => {
      const existingRow = existingRows.find((row) => row.value === candidateRow.value);
      if (!existingRow || areLookupRowsEquivalent(existingRow, candidateRow)) {
        return;
      }
      preservedChangedCount += 1;

      if (existingRow.name !== candidateRow.name) {
        sameValueNameChangedSample.push(`${existingRow.value}: ${existingRow.name} -> ${candidateRow.name}`);
      }

      if (existingRow.parentTypeId !== candidateRow.parentTypeId || existingRow.parentValue !== candidateRow.parentValue) {
        sameValueParentChangedSample.push(
          `${existingRow.value}: ${formatLookupRowParent(existingRow)} -> ${formatLookupRowParent(candidateRow)}`
        );
      }

      const attributeChanges = describeLookupAttributeChanges(existingRow, candidateRow);
      if (attributeChanges.length > 0) {
        sameValueAttributeChangedSample.push(`${existingRow.value}: ${attributeChanges.join(", ")}`);
      }
    });

    return {
      existingEntryCount: existingRows.length,
      candidateEntryCount: candidateRows.length,
      storedEntryCount: storedRows.length,
      insertedCount: insertedRows.length,
      insertedSample,
      overrideAppliedCount: overriddenRows.length,
      overriddenSample,
      preservedOmittedCount: preservedRows.length,
      omittedExistingSample: omittedExistingSample.slice(0, 5),
      preservedChangedCount,
      sameValueNameChangedCount: sameValueNameChangedSample.length,
      sameValueNameChangedSample: sameValueNameChangedSample.slice(0, 5),
      sameValueParentChangedCount: sameValueParentChangedSample.length,
      sameValueParentChangedSample: sameValueParentChangedSample.slice(0, 5),
      sameValueAttributeChangedCount: sameValueAttributeChangedSample.length,
      sameValueAttributeChangedSample: sameValueAttributeChangedSample.slice(0, 5),
    };
  }

  const replaceLookupValues = db.transaction((definition, url, _candidateContentHash, entries, checkedAt) => {
    const existingSource = statements.getLookupSource.get(definition.typeId);
    const existingRows = statements.lookupRowsByType.all(definition.typeId)
      .map((row) => normalizeStoredLookupRow(row, definition.typeId, definition.lookupName));
    const rawCandidateRows = entries
      .map((entry) => normalizeLookupRowForComparison(entry))
      .sort((left, right) => left.value.localeCompare(right.value));
    const effectiveCandidateRows = rawCandidateRows
      .map((row) => applyLookupRowOverride(row))
      .map((result) => result.row)
      .sort((left, right) => left.value.localeCompare(right.value));
    const existingByValue = new Map(existingRows.map((row) => [row.value, row]));
    const rawCandidateByValue = new Map(rawCandidateRows.map((row) => [row.value, row]));
    const effectiveCandidateByValue = new Map(effectiveCandidateRows.map((row) => [row.value, row]));
    const insertedRows = [];
    const overriddenRows = [];
    const preservedRows = [];
    const storedRowsByValue = new Map(existingRows.map((row) => [row.value, row]));

    for (const existingRow of existingRows) {
      const candidateRow = effectiveCandidateByValue.get(existingRow.value);
      if (!candidateRow) {
        preservedRows.push({
          existingRow,
        });
      ...
      const rawCandidateRow = rawCandidateByValue.get(existingRow.value);
      const overrideApplied = rawCandidateRow && !areLookupRowsEquivalent(rawCandidateRow, candidateRow);
      if (!overrideApplied || areLookupRowsEquivalent(existingRow, candidateRow)) {
        continue;
      }
      ...
      statements.updateLookupValueByTypeAndValue.run({
        typeId: storedRow.typeId,
        value: storedRow.value,
        lookupName: storedRow.lookupName,
        name: storedRow.name,
        parentTypeId: storedRow.parentTypeId,
        parentValue: storedRow.parentValue,
        code: storedRow.code,
        requiresProduct: storedRow.requiresProduct,
        isSelectLess: storedRow.isSelectLess,
        extraAttrsJson: JSON.stringify(storedRow.extraAttrs),
        updatedAt: checkedAt,
      });
      storedRowsByValue.set(storedRow.value, storedRow);
      overriddenRows.push({
        before: existingRow,
        after: storedRow,
      });
    ...
    for (const candidateRow of effectiveCandidateRows) {
      if (existingByValue.has(candidateRow.value)) {
        continue;
      }

      const rawCandidateRow = rawCandidateByValue.get(candidateRow.value);
      const overrideApplied = rawCandidateRow && !areLookupRowsEquivalent(rawCandidateRow, candidateRow);
      statements.insertLookupValue.run({
        typeId: candidateRow.typeId,
        value: candidateRow.value,
        lookupName: candidateRow.lookupName,
        name: candidateRow.name,
        parentTypeId: candidateRow.parentTypeId,
        parentValue: candidateRow.parentValue,
        code: candidateRow.code,
        requiresProduct: candidateRow.requiresProduct,
        isSelectLess: candidateRow.isSelectLess,
        ordinal: candidateRow.ordinal,
        extraAttrsJson: JSON.stringify(candidateRow.extraAttrs),
        updatedAt: checkedAt,
      });
      storedRowsByValue.set(candidateRow.value, candidateRow);
      insertedRows.push(candidateRow);
      if (overrideApplied) {
        overriddenRows.push({
          before: rawCandidateRow,
          after: candidateRow,
        });
      }
    }

    const storedRows = [...storedRowsByValue.values()].sort((left, right) => left.value.localeCompare(right.value));
    const diffDiagnostics = buildLookupMergeDiagnostics(
      existingRows,
    ...
    const storedContentHash = buildLookupStoredContentHash(storedRows);
    const changed = insertedRows.length > 0 || overriddenRows.length > 0;
    ...
    return {
      ...
      contentHash: changed ? storedContentHash : (existingSource ? existingSource.content_hash : storedContentHash),
      insertedCount: diffDiagnostics.insertedCount,
      preservedOmittedCount: diffDiagnostics.preservedOmittedCount,
      preservedChangedCount: diffDiagnostics.preservedChangedCount,
      overrideAppliedCount: diffDiagnostics.overrideAppliedCount,
      diffDiagnostics,
    };
  });
...
async function openRepository(rootDir, dataDirName = DEFAULT_DATA_DIR_NAME, options = {}) {
  const paths = getPaths(rootDir, dataDirName);
  await fs.mkdir(paths.dataDir, { recursive: true });
  ...
  const repository = createRepository(db, paths.databaseFile, options);
...
function formatLookupDiffDiagnostics(updateResult) {
  ...
  const lines = [
    `  entries: existing ${diagnostics.existingEntryCount}, candidate ${diagnostics.candidateEntryCount}, stored ${diagnostics.storedEntryCount}`,
  ];
  ...
  maybePushSample("inserted values", diagnostics.insertedCount, diagnostics.insertedSample);
  maybePushSample("omitted existing values", diagnostics.preservedOmittedCount, diagnostics.omittedExistingSample);
  maybePushSample("same-value name changes", diagnostics.sameValueNameChangedCount, diagnostics.sameValueNameChangedSample);
  maybePushSample("same-value parent changes", diagnostics.sameValueParentChangedCount, diagnostics.sameValueParentChangedSample);
  maybePushSample(
    "same-value flag/code/extraAttrs changes",
    diagnostics.sameValueAttributeChangedCount,
    diagnostics.sameValueAttributeChangedSample
  );
  maybePushSample("overridden values", diagnostics.overrideAppliedCount, diagnostics.overriddenSample);

  return lines;
}

function formatLookupDiagnosticBlock(definition, updateResult) {
  const hasStoredChanges = updateResult.insertedCount > 0 || updateResult.overrideAppliedCount > 0;
  const hasPreservedDifferences = updateResult.preservedOmittedCount > 0 || updateResult.preservedChangedCount > 0;
  if (!hasStoredChanges && !hasPreservedDifferences) {
    return [];
  }

  const header = hasStoredChanges && hasPreservedDifferences
    ? `Lookup TypeID ${definition.typeId} ${definition.lookupName} merged with preserved existing rows`
    : hasStoredChanges
      ? `Lookup TypeID ${definition.typeId} ${definition.lookupName} merged`
      : `Lookup TypeID ${definition.typeId} ${definition.lookupName} preserved existing rows`;

  return [
    header,
    ...formatLookupDiffDiagnostics(updateResult),
  ];
}
...
async function refreshLookupValues(options = {}) {
  ...
  for (const definition of LOOKUP_TYPE_DEFINITIONS) {
    ...
    const diagnosticBlock = formatLookupDiagnosticBlock(definition, updateResult);
    if (diagnosticBlock.length > 0) {
      stderr.write(`${diagnosticBlock.join("\n")}\n`);
    }
    ...
    checkedTypes.push({
      ...
      preservedOmittedCount: updateResult.preservedOmittedCount,
      preservedChangedCount: updateResult.preservedChangedCount,
      overrideAppliedCount: updateResult.overrideAppliedCount,
    ...
    if (updateResult.preservedOmittedCount > 0 || updateResult.preservedChangedCount > 0) {
      notedTypes.push({
        typeId: definition.typeId,
        lookupName: definition.lookupName,
        preservedOmittedCount: updateResult.preservedOmittedCount,
  ...
  return {
    exitCode: 0,
    reason: changedTypes.length > 0
      ? "lookup_values_changed"
      : notedTypes.length > 0
        ? "lookup_values_preserved_differences"
        : "lookup_values_unchanged",
    changedTypes,
    notedTypes,
    checkedTypes,
  };
...
async function crawlDatabase(options = {}) {
  ...
  try {
    ...
    if (options.refreshLookups !== false) {
      ...
      const insertedTypes = lookupRefreshResult.checkedTypes
        .filter((lookup) => lookup.insertedCount > 0);
      const overrideTypes = lookupRefreshResult.checkedTypes
        .filter((lookup) => lookup.overrideAppliedCount > 0);
      const preservedTypes = lookupRefreshResult.checkedTypes
        .filter((lookup) => lookup.preservedOmittedCount > 0 || lookup.preservedChangedCount > 0);

      if (insertedTypes.length > 0 || overrideTypes.length > 0 || preservedTypes.length > 0) {
        const summaryParts = [];
        if (insertedTypes.length > 0) {
          const insertedSummary = insertedTypes
            .map((lookup) => `${lookup.typeId} ${lookup.lookupName} (+${lookup.insertedCount})`)
            .join(", ");
          const typeLabel = insertedTypes.length === 1 ? "type" : "types";
          summaryParts.push(`inserted values in ${insertedTypes.length} ${typeLabel}: ${insertedSummary}`);
        }
        if (overrideTypes.length > 0) {
          const overrideSummary = overrideTypes
            .map((lookup) => `${lookup.typeId} ${lookup.lookupName} (${lookup.overrideAppliedCount})`)
            .join(", ");
          const typeLabel = overrideTypes.length === 1 ? "type" : "types";
          summaryParts.push(`applied overrides in ${overrideTypes.length} ${typeLabel}: ${overrideSummary}`);
        }
        if (preservedTypes.length > 0) {
          const preservedSummary = preservedTypes
            .map((lookup) => (
              `${lookup.typeId} ${lookup.lookupName} (omitted ${lookup.preservedOmittedCount}, changed ${lookup.preservedChangedCount})`
            ))
            .join(", ");
          const typeLabel = preservedTypes.length === 1 ? "type" : "types";
          summaryParts.push(`preserved differing existing rows in ${preservedTypes.length} ${typeLabel}: ${preservedSummary}`);
        }
        stdout.write(`lookup refresh: ${summaryParts.join("; ")}\n`);
      }
