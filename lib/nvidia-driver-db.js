...
const LOOKUP_PRODUCT_COVERAGE_NAME_ALIASES = Object.freeze({
  "NVIDIA RTX PRO 4500 Blackwell Server Edition": "NVIDIA RTX PRO 4500 Blackwell",
});
...
function createRepository(db, databaseFilePath = "") {
  ...
  function getCoverageComparableProductNames(productName) {
    const normalizedName = String(productName || "");
    const aliasName = LOOKUP_PRODUCT_COVERAGE_NAME_ALIASES[normalizedName];
    return aliasName && aliasName !== normalizedName
  ...
  function countCoveredFoundProductNames(productLookupNames) {
    ...
    for (const productName of foundProductNames) {
      if (getCoverageComparableProductNames(productName).some((candidateName) => lookupNames.has(candidateName))) {
        coveredCount += 1;
      }
    }
  ...
  function buildLookupDiffDiagnostics(typeId, entries) {
    const parseStoredExtraAttrs = (jsonText) => {
    ...
    const formatLookupDiffValue = (value) => (value === "" ? "(empty)" : String(value));
    const formatExtraAttrs = (value) => {
      const normalized = sortObjectKeys(value && typeof value === "object" && !Array.isArray(value) ? value : {});
      return Object.keys(normalized).length === 0 ? "{}" : JSON.stringify(normalized);
    };
    const describeAttributeChanges = (previousRow, nextRow) => {
      const changes = [];
      ...
      const previousExtraAttrs = formatExtraAttrs(previousRow.extraAttrs);
      const nextExtraAttrs = formatExtraAttrs(nextRow.extraAttrs);
      if (previousExtraAttrs !== nextExtraAttrs) {
        changes.push(`extraAttrs ${previousExtraAttrs} -> ${nextExtraAttrs}`);
      }

      return changes.length > 0
        ? `${previousRow.value}: ${changes.join(", ")}`
        : null;
    };
    const existingRows = statements.lookupRowsByType.all(typeId).map((row) => ({
      ...
      extraAttrs: parseStoredExtraAttrs(row.extra_attrs_json),
    }));
    ...
    for (const row of existingRows) {
      ...
      const attributeChangeDescription = describeAttributeChanges(row, nextRow);
      if (attributeChangeDescription) {
        attributeChanged.push(attributeChangeDescription);
      }
    }
...
function formatLookupDiffDiagnostics(updateResult) {
  ...
  if (
    diagnostics.addedCount === 0 &&
    diagnostics.removedCount === 0 &&
    diagnostics.renamedCount === 0 &&
    diagnostics.reparentedCount === 0 &&
    diagnostics.attributeChangedCount === 0
  ) {
    lines.push("  content hash changed but no name/value/parent/flag/extraAttrs diff was detected");
  }
...
function formatLookupDiagnosticBlock(definition, updateResult) {
  ...
  if (updateResult.changed) {
    return [
      `Lookup TypeID ${definition.typeId} ${definition.lookupName} changed`,
      ...formatLookupDiffDiagnostics(updateResult),
    ];
  }
...
async function crawlDatabase(options = {}) {
  ...
  try {
    ...
    if (options.refreshLookups !== false) {
      ...
      if (lookupRefreshResult.changedTypes.length > 0) {
        const lookupSummary = lookupRefreshResult.changedTypes
          .map((lookup) => `${lookup.typeId} ${lookup.lookupName} (${lookup.entryCount})`)
          .join(", ");
        const typeLabel = lookupRefreshResult.changedTypes.length === 1 ? "type" : "types";
        stdout.write(`lookup refresh updated ${lookupRefreshResult.changedTypes.length} ${typeLabel}: ${lookupSummary}\n`);
      }
